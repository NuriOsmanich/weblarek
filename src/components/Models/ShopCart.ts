...
export class ShopCart {
  ...
  //для удаления товара
  removeProduct(product: IProduct): boolean {
    const lengthBefore = this.cartProduct.length;
        this.cartProduct = this.cartProduct.filter((item) => item.id !== product.id);
        const changed = this.cartProduct.length < lengthBefore;
        if (changed) {
            this.changesParam();
        }
        return changed;
